...
\section{Appendix: Graphical Representations and Visual
Analysis}\label{appendix-graphical-representations-and-visual-analysis}

This appendix provides visual representations of the key mathematical
and analytical concepts discussed throughout the paper. All graphics are
generated using TikZ/PGFplots to ensure publication-quality presentation
...
Analysis}\label{calendar-spread-payoff-analysis}

Figure~\ref{fig-calendar-payoff} demonstrates the profit and loss
characteristics of the long calendar spread strategy at front-month
expiration.
...
\caption{\label{fig-calendar-payoff}Calendar spread payoff profile
showing optimal profit at strike price with limited, defined maximum
loss.}

\end{figure}%

\subsection{Greeks Evolution Analysis}\label{greeks-evolution-analysis}

Figure~\ref{fig-greeks-evolution} illustrates how the key option Greeks
evolve as the position approaches expiration and the underlying price
changes.
...
\caption{\label{fig-greeks-evolution}Evolution of option Greeks for
calendar spread positions showing risk characteristics across time and
market parameters.}

\end{figure}%

\subsection{Kelly Criterion
Optimization}\label{kelly-criterion-optimization}

Figure~\ref{fig-kelly-optimization} demonstrates the application of
Kelly criterion principles to position sizing, showing the trade-off
between growth rate and bankruptcy risk.
...
\caption{\label{fig-kelly-optimization}Kelly criterion optimization
showing optimal position sizing at 12.5\% with fractional implementation
at 3.1\% for reduced risk.}
...
Figure~\ref{fig-signal-framework} presents a flowchart of the
three-factor signal generation process used by the Sunny algorithm.
...
framework integrating Yang-Zhang volatility, term structure analysis,
and IV/RV ratio for systematic trade selection.}

\end{figure}%

\subsection{Risk Management Dashboard}\label{risk-management-dashboard}

Figure~\ref{fig-risk-dashboard} provides a comprehensive view of
portfolio risk metrics and performance characteristics.
...
\caption{\label{fig-risk-dashboard}Comprehensive risk management
dashboard showing portfolio evolution, drawdown characteristics, return
distribution (positive skew), and performance across volatility
regimes.}

\end{figure}%

\subsection{Strategy Performance Metrics
Summary}\label{strategy-performance-metrics-summary}

Table~\ref{tbl-performance-summary} provides a quantitative summary of
the key performance and risk metrics for the Sunny algorithm
implementation.

\begin{longtable}[]{@{}lcc@{}}
\caption{Sunny Algorithm Performance
Summary}\label{tbl-performance-summary}\tabularnewline
\toprule\noalign{}
