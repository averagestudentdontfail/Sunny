...
format:
  ...
  pdf:
    ...
    include-in-header:
      - text: |
          % Essential packages for document structure
          \usepackage{booktabs}
...
% Processing nodes
\node[process node, below left=of input] (yangzhang) {
    Yang-Zhang\\
    Volatility\\
    Estimator
};

\node[process node, below=of input] (termstruct) {
    Term Structure\\
    Regression\\
    Analysis
};

\node[process node, below right=of input] (ivrv) {
    IV/RV Ratio\\
    Calculation
};

% Decision diamonds
\node[decision node, below=2cm of yangzhang] (vol-check) {
    Volume\\
    $\geq 1.5M$?
};

\node[decision node, below=2cm of termstruct] (slope-check) {
    $\beta_1 \leq$\\
    $-0.00406$?
};

\node[decision node, below=2cm of ivrv] (ratio-check) {
    IV/RV\\
    $\geq 1.25$?
};

% Final decision
\node[decision node, below=3cm of input] (final-decision) {
    All Conditions\\
    Met?
};
