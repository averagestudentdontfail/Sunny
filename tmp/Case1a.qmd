...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw, thick, fill=blue!10, minimum height=1.2cm, text width=2.2cm},
  cycle/.style={rectangle, draw, thick, fill=red!20, minimum height=1.2cm, text width=2.2cm},
  diamond/.style={diamond, draw, thick, fill=orange!20, minimum height=1.2cm, text width=2.2cm, aspect=1.5},
  relief/.style={ellipse, draw, thick, fill=green!10, minimum height=1cm, text width=1.8cm},
  maintain/.style={ellipse, draw, thick, fill=red!40, minimum height=1cm, text width=1.8cm},
...
\node[diamond, below=of threat] (sensations) {Bodily Sensations\\Palpitations, Dizziness};
...
\node[box, right=2.5cm of sensations] (safety) {Safety Behaviours\\\\& Avoidance\\e.g., Fleeing store,\\Avoiding gym};
\node[relief, below=1.2cm of safety] (shortterm) {Short-Term\\Relief};
\node[maintain, above=1.2cm of safety] (longterm) {Long-Term:\\Fear Increases};
...
% Vicious cycle feedback loops
\draw[arrow, bend left=30] (anxiety) to (sensations);
\draw[arrow, bend left=40] (anxiety) to (threat);

% Safety behaviour arrows
\draw[arrow] (anxiety) -- (safety);
\draw[dashed_arrow] (safety) -- (shortterm);
\draw[arrow] (safety) -- (longterm);

% Vicious cycle highlight box
\draw[thick, red, dashed, rounded corners] 
  ([xshift=-0.4cm,yshift=0.4cm]threat.north west) rectangle 
  ([xshift=0.4cm,yshift=-0.4cm]catastrophic.south east);

\node[above=0.3cm of threat, text=red, font=\textbf\large] {Vicious Cycle};
