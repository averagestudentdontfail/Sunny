...
% Position nodes to match the Mermaid diagram's logic
\node[trigger] (A) {Trigger\\e.g., Stress,\\Exercise, Caffeine};
\node[threat, right=of A] (B) {Perceived Threat\\``Oh no, what's\\this feeling?''};
\node[sensation, below=of B] (C) {Bodily Sensations\\Palpitations,\\Dizziness};
\node[misinterpret, below=of A] (D) {Catastrophic\\Misinterpretation\\``I'm having a\\heart attack!''};
\node[anxiety, above=of D] (E) {Anxiety/Fear\\SUDS 90/100};

% Safety behaviours and outcomes are off to the side
...
\draw[arrow, bend right=45] (E.south) to (C.north);

% Arrows for the safety behaviour pathway
\draw[arrow] (E) -- (F); % The cycle leads to safety behaviours
\draw[dashed_arrow] (F) -- (G);
\draw[arrow] (F) -- (H);

% Vicious cycle highlight encompassing the core loop (C, D, E)
\draw[thick, red, dashed, rounded corners=12pt]
  ([xshift=-1.5cm, yshift=1.2cm]E.north) --
  ([xshift=1.5cm, yshift=1.2cm]E.north) --
  ([xshift=1.5cm, yshift=-1.2cm]D.south) --
  ([xshift=-1.5cm, yshift=-1.2cm]D.south) -- cycle;

% Vicious cycle label
\node[above=0.2cm of E, xshift=-1cm, text=red, font=\textbf\Large] {Vicious Cycle};
